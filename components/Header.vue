        ...
        <div class="md-modal modal-msg md-modal-transition" v-bind:class="{'md-show':loginModalFlag}">
          <div class="md-modal-inner">
            <div class="md-top">
              ...
              <button class="md-close" @click="loginhide">Close</button>
            </div>
...
export default {
  ...
  methods: {
    checklogin() {
      var _this = this;
      axois.get('/users/checklogin').then((response) => {
    ...
    login() {
      ...
      axois.post("/users/login", {
        ...
        if (res.status == '0') {
          ...
          // console.log(res.result.usersName)
          // console.log('成功')
          //
        } else {
          this.errorTip = true;
    ...
    logout() {
      var _this = this;
      axois.post('users/logout').then((response) => {
    ...
    getCartCoun() {
      var _this = this;
      axois.get('/users/getCartCount').then(function (response) {
        let res = response.data;
        if (res.status == '0') {
